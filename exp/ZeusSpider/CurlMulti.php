...
class CurlMulti {
	// url
	const TASK_ITEM_URL = 0x0001;
	// file
	const TASK_ITEM_FILE = 0x0002;
	// arguments
	const TASK_ITEM_ARGS = 0x0003;
	// operation, task level
	const TASK_ITEM_OPT = 0x0004;
	// control options
	const TASK_ITEM_CTL = 0x0005;
	// file pointer
	const TASK_FP = 0x0006;
	// success callback
	const TASK_PROCESS = 0x0007;
	// curl fail callback
	const TASK_FAIL = 0x0008;
	// tryed times
	const TASK_TRYED = 0x0009;
	// handler
	const TASK_CH = 0x000A;
	...
	private function cache($url, $content = null) {
		...
		$key = md5 ( $url );
		$dir = $this->cache ['dir'] . DIRECTORY_SEPARATOR . substr ( $key, 0, 3 ) . DIRECTORY_SEPARATOR . substr ( $key, 3, 3 );
