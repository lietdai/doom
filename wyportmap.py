...
global_options = '-sT -PE -T5 -sV --version-light -n --open --script=banner -p T:21-25,80-89,110,143,443,513,873,587,465,995,1080,1433,1521,1158,3306-3308,3389,3690,5900,6379,7001,8000-10000,27017-27019,50060,11211'

#ping test
#global_options = '-sT -PE -T5  --version-light -n --open --script=banner -p T:21-25,80-89,110,143,443,513,873,587,465,995,1080,1433,1521,1158,3306-3308,3389,3690,5900,6379,7001,8000-10000,27017-27019,50060,11211'

#no ping 
#global_options = '-sT -P0 -T5  --version-light -n --open --script=banner -p T:21-25,80-89,110,143,443,513,873,587,465,995,1080,1433,1521,1158,3306-3308,3389,3690,5900,6379,7001,8000-10000,27017-27019,50060,11211'
...
def do_port_notify(address, service, taskid = None): 
        apiHost = "127.0.0.1"
        apiPort = 8081
        httpClient = None
        try:
                params = {
                        "args":[
                                address,
                                base64.b64encode(json.dumps(service)),
                                str(taskid)
                        ]
                }
                headers = {"Content-type": "application/x-www-form-urlencoded"
                           , "Accept": "*/*"}
                #httpClient = httplib.HTTPConnection(apiHost, apiPort, timeout=30)
                #httpClient = httplib.HTTPConnection("127.0.0.1", 8080, timeout=30)
                httpClient.request("POST", "http://"+apiHost+":"+str(apiPort)+"/api/task/send-task/tasks.port_dispath", json.dumps(params), headers)
